...
type ZProductFieldMapSchemaFieldValue<T extends { required: boolean }> =
...
type ZProductFieldMapSchemaFields = {
  [K in (typeof AI_PRODUCT_DATA_MAP_GENERATOR_FIELD_NAMES)[number] as K['value']]: ZProductFieldMapSchemaFieldValue<
    Extract<
      (typeof AI_PRODUCT_DATA_MAP_GENERATOR_FIELD_NAMES)[number],
      { value: K['value'] }
    >
  >;
};

const zProductFieldMapSchemaFields =
  AI_PRODUCT_DATA_MAP_GENERATOR_FIELD_NAMES.reduce((acc, fieldName) => {
    if (fieldName.required)
      acc[fieldName.value] = z.string().describe(fieldName.description);

    if (!fieldName.required)
      acc[fieldName.value] = z
        .string()
        .nullable()
...
The target schema fields are: ${Object.keys(
  AI_PRODUCT_DATA_MAP_GENERATOR_FIELD_NAMES
).join(', ')}.
`;
