...
export async function getInitialBaseProductsWithMainImageUrlAndIsoCodeInfo(
  ...
  const [productMainImageDetectorAiResponses, productsMetricInfo] =
    await Promise.all([
      Promise.all(
        sanitizedInitialBaseProducts.map((initialBaseProduct) =>
          productMainImageDetectorAi(
            initialBaseProduct.imageUrls,
  ...
  const initialBaseProductsWithMainImageUrlAndMetricInfo =
    sanitizedInitialBaseProducts.map((initialBaseProduct, index) => {
      const mainImageHasSolidBackground =
        !productMainImageDetectorAiResponses[index].data
          ?.hasNoWhiteBackground &&
      ...
      const { currencyCode, dimensionInfo, weightInfo } =
        productsMetricInfo[index] ?? {};
  ...
  const finalInitialBaseProductsWithMainImageUrlAndMetricInfo =
    initialBaseProductsWithMainImageUrlAndMetricInfo.filter(
      (initialBaseProductWithMainImageUrlAndMetricInfo) =>
        initialBaseProductWithMainImageUrlAndMetricInfo.mainImageUrl !==
          undefined &&
        initialBaseProductWithMainImageUrlAndMetricInfo.currencyCode !==
          undefined &&
        initialBaseProductWithMainImageUrlAndMetricInfo.dimension !==
          undefined &&
...
async function getProductsMetricInfo(
  headerLine: string,
  productDataLines: string[]
) {
  ...
  const productMetricInfo = productMetricInfoResponses.map(
